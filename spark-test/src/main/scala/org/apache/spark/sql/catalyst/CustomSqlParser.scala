...
object CustomSqlParser extends AbstractSparkSQLParser with DataTypeParser {

  def checkParse(sql: String): org.apache.spark.sql.catalyst.ParseResult = {
    initLexical
    val scanner = new lexical.Scanner(sql)
    val result = phrase(start)(scanner)
    result match {
      case Success(plan, _) => ParseSucceed(plan)
      case Failure(msg, next) => ParseFailed(translate(msg), next.pos.line, next.pos.column)
      case Error(msg, next) => ParseFailed(msg, next.pos.line, next.pos.column)
    }
  }

  val failureMap = Map[String, String](
    "unclosed comment" -> """注释不完整。"/*"开头的注释需要以"*/"结尾""",
    "unclosed string literal" -> """字符串不完整。字符串需要包括在成对的''或""内""",
    "end of input" -> "语句不完整。请检查语句",
    "end of input expected" -> "语句应当于此处终止，但未终止。",
    "illegal character" -> "非法字符")

  def translate(msg: String): String = failureMap.getOrElse(msg, msg)
